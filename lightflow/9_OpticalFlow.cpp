...
int maxCount = 20;	// 检测的最大特征数，原值500
	...
	if(capture.isOpened())	// 摄像头读取文件开关
	{
		while(true)
		{
			...
			{ 
				...
				GaussianBlur(frame, frame, Size(SIZE, SIZE), 0, 0);
...
void tracking(Mat &frame, Mat &output)
{
	...
	Mat result;
	//calcOpticalFlowSF(gray_prev, gray, result, 3, 2, 4);
	/*calcOpticalFlowFarneback(gray_prev, gray, result, 0.5, 3, 5, 3, 5, 1, 0);
	Vec2f point;
	for (int i = 0; i < result.rows; i++)
	{
		for (int j = 0; j < result.cols; i++)
		{
			point = result.at<Vec2f>(i, j);
